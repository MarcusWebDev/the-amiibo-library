...
function App() {
  ...
  const toggleSelectedAmiiboCollection = async (selectedIDs) => {
    ...
    await fetch("https://api.amiibolibrary.com:443/collection", {
      method: "POST",
  ...
  useEffect(() => {
    if (user != null) {
      fetch("https://api.amiibolibrary.com:443/signIn", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({
          user: user
      ...
      .then(() => {
        return fetch(`https://api.amiibolibrary.com:443/amiibo/${user.email}`, {
          method: "GET"
        });
      })
      .then(async (response) => {
